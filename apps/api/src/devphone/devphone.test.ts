...
describe('/dev/phone', () => {
  ...
  it('/dev/phone/send runs the same inbound pipeline as the webhook', async () => {
    ...
    const send = await app.inject({
      method: 'POST',
      url: '/dev/phone/send',
      payload: { phone: '+15559990000', body: 'hi vector' },
    });
    ...
    expect(body.messages.some((m: { body: string }) => m.body === 'hi vector')).toBe(true);
  });

  it('/public/vector.vcf serves a vCard', async () => {
    const { app } = await makeTestApp();
    const res = await app.inject({ method: 'GET', url: '/public/vector.vcf' });
    expect(res.statusCode).toBe(200);
