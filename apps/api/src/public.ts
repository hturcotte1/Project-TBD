/** `GET /public/vector.vcf` — the agent's contact card, publicly fetchable (no auth). */
import type { FastifyInstance } from 'fastify';
import { buildVCard } from '@apogee/messaging';
import type { ApiDeps } from './deps';

export function registerPublicRoutes(app: FastifyInstance, deps: ApiDeps): void {
  app.get('/public/vector.vcf', async (_req, reply) => {
