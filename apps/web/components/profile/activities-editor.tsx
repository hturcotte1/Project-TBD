...
/**
 * A plain add/remove list for the up-to-10 Common App activities. The onboarding area has its own
 * activities editor, but it still depends on pre-migration shim components another area is still
 * restyling — this is a from-scratch rebuild on the component system instead of reusing it unchanged.
 */
export function ActivitiesEditor({ activities, onChange }: { activities: ActivityInput[]; onChange: (next: ActivityInput[]) => void }) {
  ...
  return (
    <div className="flex flex-col gap-6">
      {activities.length === 0 ? <p className="text-14 text-fg-2">No activities yet.</p> : null}
      ...
      {activities.map((activity, index) => (
        <div key={index} className="flex flex-col gap-3">
          ...
          <Field label="Description" help={`${150 - activity.description.length} characters left`}>
            <Textarea value={activity.description} maxLength={150} rows={2} onChange={(event) => update(index, { description: event.target.value })} placeholder="What you did, Common App style." />
          </Field>
      ...
      <div className="flex flex-col gap-1">
        <Button variant="text" size="sm" className="h-auto w-fit px-0" disabled={activities.length >= MAX_ACTIVITIES} onClick={() => onChange([...activities, emptyActivity()])}>
          <Plus /> Add activity
        </Button>
        {activities.length >= MAX_ACTIVITIES ? <p className="text-12 text-fg-2">Common App caps activities at {MAX_ACTIVITIES}.</p> : null}
      </div>
    </div>
  );
}
